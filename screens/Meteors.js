        ...
        return (
         <View>
            <ImageBackground source={bg_img} style={styles.BackgroundImage}>
                ...
                {marginTop: 400,
                marginLeft: 50 }]}>{item.name}</Text>
                <Text style={[styles.cardText,
                { marginTop: 5, marginLeft: 50 }]}>Mas cercano a la tierra -

                {item.close_approach_data[0].close_approach_data_full}
                </Text>
                <Text style={[styles.cardText,
                { marginTop: 5, marginLeft: 50 }]}>Diametro minimo (KM) -
                {item.estimated_diameter.kilometers.estimated_diameter_min}
                </Text>
                <Text style={[styles.cardText,
                { marginTop: 5, marginLeft: 50 }]}>Diametro maximo (KM) -
                {item.estimated_diameter.kilometers.estimated_diameter_max}
                </Text>
                <Text style={[styles.cardText,
                { marginTop: 5, marginLeft: 50 }]}>Velocidad (KM/H) -
                {item.close_approach_data[0].relative_velocity.kilometers_per_hour}
                </Text>
                <Text style={[styles.cardText,
                { marginTop: 5, marginLeft: 50 }]}>alejandose de la tierra por (KM) -
